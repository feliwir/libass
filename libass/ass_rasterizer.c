 ...
 */
static inline int check_capacity(ASS_Rasterizer * rst, int index,
                                 size_t delta)
{
    ...
    while (capacity < delta)
        capacity *= 2;
    void *ptr =
        realloc(rst->linebuf[index], sizeof(struct segment) * capacity);
    if (!ptr)
        return 0;
 ...
 */
static inline int add_line(ASS_Rasterizer * rst, OutlinePoint pt0,
                           OutlinePoint pt1)
{
    ...
    line->c *= 1 << shift;
    line->scale =
        (uint64_t) 0x53333333 *(uint32_t) (max_ab *
...
static int add_quadratic(ASS_Rasterizer * rst,
                         OutlinePoint pt0, OutlinePoint pt1,
                         OutlinePoint pt2)
{
    OutlineSegment seg;
...
static void segment_split_horz(struct segment *line, struct segment *next,
                               int32_t x)
{
...
static void segment_split_vert(struct segment *line, struct segment *next,
                               int32_t y)
{
...
static inline int segment_check_left(const struct segment *line, int32_t x)
{
    ...
    int64_t cc = line->c - line->a * (int64_t) x -
        line->b *
        (int64_t) (line->flags & SEGFLAG_UL_DR ? line->y_min : line->y_max);
    if (line->a < 0)
        cc = -cc;
    return cc >= 0;
}
...
static inline int segment_check_right(const struct segment *line, int32_t x)
{
    ...
    int64_t cc = line->c - line->a * (int64_t) x -
        line->b *
        (int64_t) (line->flags & SEGFLAG_UL_DR ? line->y_max : line->y_min);
    if (line->a > 0)
        cc = -cc;
    return cc >= 0;
}
...
static inline int segment_check_top(const struct segment *line, int32_t y)
{
    ...
    int64_t cc = line->c - line->b * (int64_t) y -
        line->a *
        (int64_t) (line->flags & SEGFLAG_UL_DR ? line->x_min : line->x_max);
    if (line->b < 0)
        cc = -cc;
    return cc >= 0;
}

static inline int segment_check_bottom(const struct segment *line,
                                       int32_t y)
{
    ...
    int64_t cc = line->c - line->b * (int64_t) y -
        line->a *
        (int64_t) (line->flags & SEGFLAG_UL_DR ? line->x_max : line->x_min);
    if (line->b > 0)
        cc = -cc;
    return cc >= 0;
}
...
static int polyline_split_horz(const struct segment *src, size_t n_src,
                               struct segment **dst0, struct segment **dst1,
                               int32_t x)
{
    int winding = 0;
                                             ...
                                             int64_t c, int32_t scale)
{
    ...
    for (j = 0; j < height; ++j) {
        for (i = 0; i < width; ++i) {
            int64_t cc =
                c - (a * (int64_t) i +
                     b * (int64_t) j) * (1 << (rst->tile_order + 6));
            int64_t offs_c = offs - cc;
            int64_t abs_c = offs_c < 0 ? -offs_c : offs_c;
            if (abs_c < size)
                rst->fill_halfplane(buf + i * step, stride, a, b, cc,
                                    scale);
            else
                rst->fill_solid(buf + i * step, stride,
                                ((uint32_t) (offs_c >> 32) ^ scale) &
                                0x80000000);
        }
        buf += tile_stride;
    }
...
static int rasterizer_fill_level(ASS_Rasterizer * rst,
                                 uint8_t * buf, int width, int height,
                                 ptrdiff_t stride, int index, size_t offs,
                                 int winding)
{
    ...
    if (width == 1 << rst->tile_order && height == 1 << rst->tile_order) {
        rst->fill_generic(buf, stride, line, rst->size[index] - offs,
                          winding);
        rst->size[index] = offs;
        return 1;
    }
    ...
    if (width > height) {
        ...
        buf1 += width;
        winding1 +=
            polyline_split_horz(line, n, &dst0, &dst1,
                                (int32_t) width << 6);
    } else {
        ...
        buf1 += height * stride;
        winding1 +=
            polyline_split_vert(line, n, &dst0, &dst1,
                                (int32_t) height << 6);
    }
    rst->size[index ^ 0] = dst0 - rst->linebuf[index ^ 0];
    rst->size[index ^ 1] = dst1 - rst->linebuf[index ^ 1];

    if (!rasterizer_fill_level
        (rst, buf, width, height, stride, index ^ 0, offs, winding))
        return 0;
    assert(rst->size[index ^ 0] == offs);
    if (!rasterizer_fill_level
        (rst, buf1, width1, height1, stride, index ^ 1, offs1, winding1))
        return 0;
...
int rasterizer_fill(ASS_Rasterizer * rst,
                    uint8_t * buf, int x0, int y0, int width, int height,
                    ptrdiff_t stride)
{
    assert(width > 0 && height > 0);
    assert(!(width & ((1 << rst->tile_order) - 1)));
    ...
    if (rst->y_min <= 0) {
        ...
        struct segment *dst1 = rst->linebuf[index ^ 1];
        winding =
            polyline_split_vert(rst->linebuf[index], n, &dst0, &dst1, 0);
        index ^= 1;
        n = dst1 - rst->linebuf[index];
    }
    rst->size[index] = n;
